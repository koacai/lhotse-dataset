...
class CallFriendJP(BaseCorpus):
    def __init__(self, email: str, pswd: str) -> None:
        super(CallFriendJP, self).__init__()
        self.email = email
        self.pswd = pswd

    @property
    def url(self) -> str:
        return "https://ca.talkbank.org/access/CallFriend/jpn.html"
    ...
    @property
    def download_page_url(self) -> dict[str, str]:
        return {
            "normal": "https://media.talkbank.org/ca/CallFriend/jpn/0wav",
            "extra": "https://media.talkbank.org/ca/CallFriend/jpn/0extra/0wav",
        }
    ...
    def get_cuts(self) -> Generator[lhotse.MultiCut, None, None]:
        ...
        for download_url in self.download_page_url.values():
            ...
            with tempfile.TemporaryDirectory() as tmp_dir:
                for link in links:
                    audio_id = f"callfriend_jp_{Path(link).stem}"
